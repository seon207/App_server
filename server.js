...
var clients = [];
var u_id, u_pass;   //아이디, 비밀번호, 병원코드, 환자코드
...
io.on('connection', function (socket) { //서버 연결
    ...
    socket.on('signup_check', function (data) {
        ...
        connection.query(testQuery, [h_code, p_code], function (err, results, fields) {
            ...
            if (results[0] == undefined) {   //병원코드, 환자코드 일치하는 환자 없을 때
                ...
                for (var data of results) { //다른방법 없으려나
                    var u_name = data.NAME;
                }
                pcheck = true;
                data.name = u_name;
            }
    ...
    socket.on('SignUp', function (data) {   //회원가입
        ...
        if (icheck == true && pcheck == true) { //환자정보 확인 버튼 누른 경우
            console.log("들어옴");
            ...
            connection.query(testQuery, [h_code, u_id, hashpw, p_code, 1], function (err, results, fields) {
                if (err) {
                    console.log(err);
                }
                data.signup = 1;
